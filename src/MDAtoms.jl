...
# @properties Atoms begin
# 	natoms(atoms) = size(getfield(atoms, :positions), 1)
# 	volume(atoms) = prod(diag(getfield(atoms, :lattice)))
# end
...
function calculate_distances(atoms::Atoms, atom_index::Integer)
	...
	position_differences = atoms.positions .- atom_position
